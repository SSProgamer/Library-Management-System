...
CREATE TABLE `Borrow_return` (
  ...
  PRIMARY KEY (`BR_ID`),
   KEY `memberid_FK` (`Member_ID`),
  CONSTRAINT `memberid_FK` FOREIGN KEY (`Member_ID`) REFERENCES `Member` (`Member_ID`) ON DELETE CASCADE,
   KEY `bookid_FK` (`Book_ID`),
  CONSTRAINT `bookid_FK` FOREIGN KEY (`Book_ID`) REFERENCES `Book_List` (`Book_ID`) ON DELETE CASCADE,
  KEY `libid_FK` (`Lib_ID`),
  CONSTRAINT `libid_FK` FOREIGN KEY (`Lib_ID`) REFERENCES `Librarian` (`Lib_ID`) ON DELETE CASCADE,
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
VALUES (1,1,1,'2565-02-23','2565-02-24','อยากนอนห้องนี้เลยจอง',1),(2,2,2,'2565-02-23','2565-02-24','กุว่างจบปะบรรณารักษ์ 1-1ได้อะ',2)
